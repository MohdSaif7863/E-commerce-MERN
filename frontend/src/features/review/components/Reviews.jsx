...
export const Reviews = ({productId,averageRating}) => {
    ...
    // Filter reviews first, then calculate rating counts
    const validReviews = reviews?.filter(review => review && review.rating) || []
    
    validReviews.forEach((review)=>{
        if (review && review.rating && ratingCounts.hasOwnProperty(review.rating)) {
  ...
  return (
        <Stack rowGap={5} alignSelf={"flex-start"}  width={is480?"90vw":is840?"25rem":'40rem'}>
                    ...
                    validReviews?.length?(
                        <Stack rowGap={3}>
                            ...
                            <Stack rowGap={1} >
                                ...
                                <Typography variant='h6' color={'text.secondary'} >Based on {validReviews.length} {validReviews.length===1?"Review":"Reviews"}</Typography>
                            </Stack>
                            ...
                            <Stack rowGap={2}>
                                {
                                    [5,4,3,2,1].map((number)=>(
                                        <Stack flexDirection={'row'} justifyContent={'space-between'} alignItems={'center'} columnGap={1}>
                                            ...
                                            <LinearProgress sx={{width:"100%",height:"1rem",borderRadius:"4px"}} variant='determinate' value={(ratingCounts[number]/validReviews?.length)*100}/>   
            ...
            {/* reviews mapping */}
            <Stack rowGap={2} >
                {reviews?.filter(review => review && review._id && review.user && review.user._id).map((review)=>(<ReviewItem key={review._id} id={review._id} userid={review.user._id} comment={review.comment} createdAt={review.createdAt} rating={review.rating} username={review.user.name} />))}
            </Stack>
