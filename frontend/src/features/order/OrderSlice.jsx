...
import { createOrder, getAllOrders, getOrderByUserId, updateOrderById, deleteOrderById } from './OrderApi'
...
export const deleteOrderByIdAsync=createAsyncThunk("orders/deleteOrderByIdAsync",async(id)=>{
...
const orderSlice=createSlice({
    ...
    extraReducers:(builder)=>{
        builder
            ...
            .addCase(deleteOrderByIdAsync.pending,(state)=>{
                state.orderDeleteStatus='pending'
            })
            .addCase(deleteOrderByIdAsync.fulfilled,(state,action)=>{
                state.orderDeleteStatus='fulfilled'
                state.orders=state.orders.filter((order)=>order._id!==action.payload.id)
            })
            .addCase(deleteOrderByIdAsync.rejected,(state,action)=>{
                state.orderDeleteStatus='rejected'
                state.errors=action.error
            })
    }
})
...
// exporting reducers
export const {resetCurrentOrder,resetOrderUpdateStatus,resetOrderFetchStatus,resetOrderDeleteStatus}=orderSlice.actions
...
export const selectOrderDeleteStatus=(state)=>state.OrderSlice.orderDeleteStatus
